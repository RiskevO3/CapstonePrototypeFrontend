...
import ViewRfqBid from './Modal/ViewRfqBid.vue'
...
<template>
  <ViewRfqBid v-model="isModalActive" :item-detail="rfqItemDetail"/>
  ...
  <table>
    ...
    <tbody>
      <tr v-for="item in itemsPaginated" :key="item.id">
        ...
        <BaseButton class="w-full" :label="item.status" :color="item.status == 'Pending' ? 'info' : item.status == 'Approved' ? 'success' : 'error'" type="button"/>
        </td>
